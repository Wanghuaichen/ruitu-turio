...
{
  w5500_write_byte(RTR0,(uint8_t)((timeout & 0xff00) >> 8));
  w5500_write_byte(RTR1,(uint8_t)(timeout & 0x00ff));
}
...
{
  w5500_write_byte(WIZ_RCR,retry);
}
...
{
  w5500_write_byte(IR, ~mask | getIR() );
}
...
{
  w5500_write_byte( Sn_MSSR0(s), (uint8_t)((Sn_MSSR & 0xff00) >> 8));
  w5500_write_byte( Sn_MSSR1(s), (uint8_t)(Sn_MSSR & 0x00ff));
}
...
uint16_t getSn_TX_FSR(SOCKET s)
{
  ...
  do
  {
    val1 = w5500_read_byte(Sn_TX_FSR0(s));
    val1 = (val1 << 8) + w5500_read_byte(Sn_TX_FSR1(s));
      if (val1 != 0)
    {
        val = w5500_read_byte(Sn_TX_FSR0(s));
        val = (val << 8) + w5500_read_byte(Sn_TX_FSR1(s));
    }
  } while (val != val1);
   return val;
}
...
void send_data_processing(SOCKET s, uint8_t *data, uint16_t len)
{
  ...
  ptr += len;
  w5500_write_byte( Sn_TX_WR0(s) ,(uint8_t)((ptr & 0xff00) >> 8));
  w5500_write_byte( Sn_TX_WR1(s),(uint8_t)(ptr & 0x00ff));
}
...
void socket_buf_init( uint8_t * tx_size, uint8_t * rx_size  )
{
  ...
  {
    w5500_write_byte( (Sn_TXMEM_SIZE(i)), tx_size[i]);
    w5500_write_byte( (Sn_RXMEM_SIZE(i)), rx_size[i]);

#ifdef __DEF_IINCHIP_DBG__
//         printf("tx_size[%d]: %d, Sn_TXMEM_SIZE = %d\r\n",i, tx_size[i], w5500_read_byte(Sn_TXMEM_SIZE(i)));
//         printf("rx_size[%d]: %d, Sn_RXMEM_SIZE = %d\r\n",i, rx_size[i], w5500_read_byte(Sn_RXMEM_SIZE(i)));
#endif
    SSIZE[i] = (int16)(0);
    RSIZE[i] = (int16)(0);
